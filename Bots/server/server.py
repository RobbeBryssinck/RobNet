...
class C2Servicer(c2_pb2_grpc.C2Servicer):
    ...
    def StartJob(self, request, context):
        print("Received start job")
        response = self.server_manager.start_job(request)
        print("Done start job")
        return response
...
if __name__ == "__main__":
    job_server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
